...
function Settings(props) {
    ...
    function validateEmail(username) {
        if (username.trim().match(/^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i) == null) {
            setAlert(true);
            setErrorType('iemail');
    ...
    async function LoginBtn(e) {
        ...
        if (!alert && username.length != 0 && password.length != 0) {
            ...
            let res = await fetch(process.env.REACT_APP_API_BASE + '/account/rest-api/login', body);
            ...
            <Grid container>
                    ...
                    <form class="form">
                        ...
                        <div className="formTextboxContainer">
                            <div class="wrap-input100 rs1-wrap-input100 validate-input m-b-20" data-validate="Type user name">
                                ...
                                <input id="email" variant="outlined" className="input100 textbox" type="text" name="pass" placeholder="email@provider.com" value={username} onChange={(e) => { setUsername(e.target.value); validateEmail(e.target.value) }} />
                            </div>
                            <div class="wrap-input100 rs2-wrap-input100 validate-input m-b-20" data-validate="Type password">
                                <span className='fieldHeader'>Password</span>
                                <Link className='fieldHeaderForgot' to='/passrecovery'>Forgot Password</Link>
                                <input id="password" onChangeCapture={(e) => { passwordValidate(e.target.value) }} variant="outlined" className="input100 textbox" type="text" name="pass" placeholder="Password" value={password} onChange={(e) => { setPassword(e.target.value) }} />
                            </div>

                            <div class="container-login100-form-btn">
                                <MyAlert />
                                <button disabled={buttonDisable} id='loginButton' class="login100-form-btn" onClick={LoginBtn}>Sign in</button>
                            </div>
                        </div>
                        <div className="signUpContainer">
                            <div class="w-full  signup">Don't have a Privyplay account?</div>
                            <span className='signUpBtn'><Link to='/register'>Sign Up!</Link></span>
                        </div>
                ...
                <Grid item xs={6} className='imgRoot'>
                    <div className="imgBackround">
                        <span className="imgHeadingLogin">The freedom to express yourself in videos even
                        more using faster-playing embeds with accesscontrol options!
                        </span>
                        <div class="mainImg" >
